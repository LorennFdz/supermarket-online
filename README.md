# SuperMarket Online | Full Stack E-commerce

Este proyecto consiste en una plataforma de comercio electrónico integral, diseñada con un enfoque en la eficiencia del lado del cliente y una navegación fluida. El sistema gestiona un catálogo dinámico de más de 100 productos, permitiendo una experiencia de compra interactiva y moderna.

## 🚀 Características Técnicas

* **Gestión de Estado Reactiva:** Implementación de un carrito de compras funcional con persistencia de datos y actualización en tiempo real de cantidades y totales.
* **Filtrado Avanzado:** Sistema de búsqueda y filtrado dinámico por categorías, marcas y rangos de precio, optimizando la recuperación de información desde el cliente.
* **Interfaz de Usuario (UI):** Diseño minimalista y organizado, con una arquitectura de componentes reutilizables y navegación intuitiva.
* **Arquitectura de Datos:** Consumo de estructuras de datos JSON para simular un entorno de producción, facilitando una futura integración con APIs RESTful.

## 🛠️ Tecnologías Utilizadas

* **Frontend:** React.js, JavaScript (ES6+).
* **Estilos:** CSS3.
* **Herramientas:** Git para control de versiones y Vercel para el despliegue continuo.

## ⚙️ Próximos Pasos

Como parte de mi proceso de mejora continua, este proyecto está evolucionando hacia:
1. **Migración a Backend:** Implementación de una API robusta con **Java y Spring Boot**.
2. **Capa de Persistencia:** Integración con **PostgreSQL / MySQL**.
3. **QA & Testing:** Incorporación de pruebas unitarias con **JUnit** para asegurar la integridad de la lógica de negocio.